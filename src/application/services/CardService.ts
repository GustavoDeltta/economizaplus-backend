...
export class CardService {
  ...
  async updateCard(
    ...
    last4Digits: string,
    limitTotal: Decimal | null,
    limitRemaining: Decimal | null | undefined,
    closingDay: number | null,
    dueDay: number | null,
    type: string,
  ) {
  ...
  async payInvoice(userId: string, cardId: string, amount: Decimal) {
    const card = await this.cardRepository.findCard(cardId);
    if (!card) throw new BadRequestError("Cartão não encontrado");
    if (card.userId !== userId)
      throw new BadRequestError("Cartão não pertence ao usuário");
    if (card.type !== "CREDIT")
      throw new BadRequestError("Somente cartões de crédito possuem fatura");

    const total = card.limitTotal ?? new Decimal(0);
    const current = card.limitRemaining ?? new Decimal(0);

    const newRemaining = Decimal.min(current.add(amount), total);

    return this.cardRepository.updateLimit(cardId, newRemaining);
  }
}
