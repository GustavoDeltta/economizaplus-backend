    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (dto.type === "TRANSFER") {
        if (!dto.destinationWalletId) {
          throw new BadRequestError(
            "ID da carteira de destino é obrigatório para transferências.",
          );
        }
        const destination = await this.walletRepository.findById(
        ...
        await this.walletRepository.updateBalance(
          wallet.id!,
          wallet.balance.minus(totalAmount),
          tx,
        );
        // Credit destination
        await this.walletRepository.updateBalance(
          destination.id!,
          destination.balance.add(totalAmount),
          tx,
        );
        ...
        const outTransaction = new Transaction(
          ...
          goal_id,
          cardId,
          description || `Transferência para ${destination.name}`,
        );
        ...
        const inTransaction = new Transaction(
          ...
          goal_id,
          cardId,
          description || `Transferência de ${wallet.name}`,
        );
      ...
      if (dto.type === "INCOME") {
        ...
        if (dto.paymentMethod !== "CREDIT_CARD") {
          ...
          if (!dto.cardId) {
            throw new BadRequestError(
              "ID do cartão é obrigatório para pagamento com cartão de crédito.",
            );
          }
          ...
          if (card.limitRemaining === null) {
            throw new BadRequestError(
              "O cartão selecionado não possui limite configurado.",
            );
          }
          ...
          await this.cardRepository.updateLimit(
            card.id!,
            card.limitRemaining.minus(totalAmount),
            tx,
          );
        }
      }
      ...
      if (
        dto.type === "INCOME" ||
        (dto.type === "EXPENSE" && dto.paymentMethod !== "CREDIT_CARD")
      ) {
      ...
      if (
        ...
        const installmentAmount = totalAmount
          .dividedBy(dto.totalInstallments)
          .toDecimalPlaces(2);
        ...
        for (let i = 1; i <= dto.totalInstallments; i++) {
          ...
          if (i === dto.totalInstallments) {
            const usedTotal = installmentAmount.times(
              dto.totalInstallments - 1,
            );
          ...
          if (i === 1) {
            firstTransaction = await this.transactionRepository.create(
              transaction,
              tx,
            );
          } else {
        ...
        if (transaction.type === "INCOME") {
          ...
          if (transaction.paymentMethod !== "CREDIT_CARD") {
            ...
            if (card && card.limitRemaining !== null) {
              await this.cardRepository.updateLimit(
                card.id!,
                card.limitRemaining.add(transaction.amount),
                tx,
              );
            }
          }
